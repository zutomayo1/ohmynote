...
- **数学公式与 Mermaid 图**：`$$…$$` / `\[…\]` / `\(…\)` 走 KaTeX，` ```mermaid ` 代码块画流程图 / 时序图；
  两个库**自托管在 `app/static/vendor/`**（不依赖 CDN，离线可用），且只在正文真的包含时才按需加载。
- **键盘导航**：列表页 / 搜索结果 `J`/`K`（或 `↓`/`↑`）选中、`Enter` 打开、`X` 批量勾选；任何页面按 `?` 呼出快捷键速查浮层。
...
- **关系图谱**（`app/services/graph.py` + `static/js/graph.js` + `templates/graph.html`）：
  - **后端**：`build_graph()` 把 `note_links` 抽成 `{nodes, edges}`（只保留两端都存在的边），
    节点带 `degree / in_degree / out_degree / community / tag_color`；`louvain()` 是纯 Python 的
    **Louvain 社区检测**（确定性：迭代顺序按度降序、候选含「留在原社区」且仅在严格更优时移动——
    少了后半条会在三角形这类对称图上震荡不收敛），社区 id 按规模降序，方便前端「前 8 个簇各配一色」。
    `ego_graph()` 出**邻域子图**（1–2 跳，超限时按连通度+更新时间截断并标 `truncated`）。
...
- **用量统计**：今日/本周/本月/累计四窗口 + 环比，成功率与耗时，最近 20 次调用明细（含失败原因），按任务（问笔记/流式/笔记助手分开归因）、按模型、模型×任务、时间规律（小时/星期）、近 6 个月趋势、30 天堆叠柱状图（网格刻度、进场动画、悬停/方向键看当天明细）、14 天明细表，可导出 CSV
...
| **笔记助手（Agent）** | 导航「更多 → 笔记助手」 | 用一句话让它多步操作：搜（关键词/标签/分类/近 N 天）、读写笔记与摘要、标签增删、分类、归档 / 置顶 / 加星 / 公开博客 / 回收站恢复、笔记库统计，执行过程逐步实时汇报（SSE）；可开「只读模式」只查不改；写操作有版本历史可回退，删除只走回收站 |
| **写作热力图** | 统计页 | GitHub 风格年度贡献格，一眼看到写作习惯 |
| **专注模式** | 编辑器「专注」按钮 | 一键隐去导航与设置，Esc 退出 |
| **单文件导出** | 笔记详情「导出网页」 | 自包含排版 HTML，发给别人即开即读 |
| **配色预设** | 导航「更多」 | 墨迹 / 竹青 / 墨蓝三套，亮暗自动适配 |

| **夜间整理** | 自动（需配 AI，`INKNOTE_TIDY=0` 关闭） | 每天自动给未分类笔记补分类、把「收件箱」标签换成「已归档」 |
| **问笔记多跳检索** | /ask 自动生效 | 资料不够时 AI 会自动追加更精确的检索（最多 2 跳）再回答，复杂问题更准 |
...
- **双链按标题精确匹配**（忽略大小写）：同名笔记只会连到最近更新的那一篇。**改标题时编辑页会提示「还有 N 篇引用着旧标题」并提供一键改写**（`POST /notes/{id}/rename-links`，走 `repo.rename_link_refs`：别名保留、代码块不动、逐篇走 update_note 留版本快照）；新标题与其它笔记撞名时会拒绝并提示。数据库里已解析到该笔记的反向链接仍保留，`repo._attach_dangling_links` 只会把此前悬空的链接接到新建/改名后的标题上。
- **全文索引自带中文二元分词**（`search.py` 的 `_index_text`）：CJK 切成「单字 + 相邻二元组」存进 FTS5（unicode61），**任何长度的中文查询都走索引**，不再有「短词退回 LIKE」的慢路径；旧 trigram 索引在启动时自动检测并全量重建一次。搜索支持轻量语法（`tag:` / `title:` / `cat:` / `is:` / `after:` / `before:` / 引号短语 / `-排除`，见 `parse_query`）。**仍未做**：模糊匹配、同义词、真正的中文分词（切出「词」而不是字组）。
...
- **关系图谱的边界**：有局部图与全图、缩放平移、节点搜索、按连接数缩放、按聚类/标签着色。
  仍是自研的轻量实现，所以没有：力导向参数面板、按节点类型分组着色（Obsidian 的 groups）、
  导出图片、3D。聚类是 Louvain 单层结果，不保证与你心里的分类一致。
  `moveSelection`（方向键选点）是方向启发式，不保证「视觉上最近」。
  超长标题会挤在一起（碰撞半径里给了 60% 标题半宽，不是全避让）。
  局部图默认最多 60 个节点（超出按连通度截断，页面会标明「图里只画了 N 篇」）。
