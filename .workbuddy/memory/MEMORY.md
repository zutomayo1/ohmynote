...
- 全量：`cd C:/repo/inknote && ./.venv/Scripts/python.exe -m pytest tests -q -n 8 --dist loadfile --junitxml=.scratch/junit.xml`（约 20s；loadfile 不能省，否则共享 session 库拆散出假红；沙箱吞退出码，权威结果解析 junitxml；临时目录别指项目内）。
...
- **图形/布局类改造**：零依赖 ⇒ 借算法不引库。Louvain 等局部移动算法必须「候选含原状态 + 严格更优才移动」，否则对称图震荡不挂死；指针拖拽别用 setPointerCapture（click 会跑到 <svg>）；力布局拖完要短时「钉住」节点，否则被弹簧拽回。
- **e2e 断言**：临时库带着真实数据 ⇒ 只统计本测试造的记录；读数前先「等模拟停稳」；观感用 getComputedStyle 量化，别靠截图肉眼判断。
...
- 后台线程总开关 `INKNOTE_WORKERS`，conftest 设 0；测线程自设 1。
...
- 记账唯一入口 `ai.chat()` 的 `_record_usage()`；不走 chat() 的路径（流式 /ask/stream 等）必须自己记，流式 usage 在 `if not choices: continue` 之前取。
- 失败也记账（ok=0 + 原因）；task 名区分调用方，新调用点同步 `ai_admin.py` 的 TASK_LABELS。
- ai_usage 表自管列（`ai_usage.COLUMNS`，ensure() 按 PRAGMA 补 ALTER）；`summary()` 只加键不改键。

## 前端守卫与惯例
- 样式双守卫：audit_css.py（SSR 模板）+ test_js_class_guard.py（JS 造的类名，KNOWN_UNSTYLED 只许变短）。
- CSS 按轮次追加：加规则前先 grep 同选择器（优先级坑曾让下划线偏 14px）；删补丁节用双锚点，绝不能 `src[:start]`（曾连带删掉后 63 行）；入场动画用 `backwards` 不用 `both`（残留 transform 困住浮层 z-index）。
- `.is-empty` 带 pointer-events:none，加到可交互元素会静默失效；排查「悬停没反应」用 elementFromPoint + getComputedStyle。
- 图标按钮必须 title + aria-label；图标先确认 `_macros.html` 定义过；新页面必须有导航入口（/stats 曾整站无链接）；动作区控件同高（36/30/44）；低频动作收 `.menu-group`。
- 接口冻结在 `docs/roundN-*.md`；类名与 id 以 `docs/frontend-contract.md` 为准。

## 自绘下拉（select.js，全站 10 个已改）
- 只换视觉不动布局：原生 select 留在文档流当尺寸基准（决定宽度），加 tabindex=-1/aria-hidden/pointer-events:none，自绘按钮 absolute inset:0 盖上；选中后 dispatchEvent(change) 让自动提交/批量显隐继续生效；渐进增强，无 JS 时 SSR 不变。
- 坑：select 换块级盒子后退回 shrink-to-fit，必须 `.select-combo .select-combo__native { display:block; width:100% }`。

## 悬浮提示（chart-tip.js，全站自绘浮层）
- 命中选择器 `[data-tip-title], [title], [data-tip-stash-title]`：原生 title 自动桥接，模板不用改；title 一进元素立刻摘到 stash（防原生气泡抢跑——延迟窗口里浏览器气泡会先弹出，尤其刚展示过一次后），离开还原（无 JS 兜底仍在）。
...
- 面板 width:max-content 紧贴文字，别加 min-width；上限 300px；label nowrap（竖排折行根因）。
...
- **先量再改**：几何基线脚本 + 亮暗双主题截图；造数据故意放超长文本逼出边界分支；「重叠」必须视口内同时可见才算数（关闭的 details 子元素仍有布局盒）。
